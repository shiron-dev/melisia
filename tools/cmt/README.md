...
`apply` は対象**リモートホスト上**に project 単位のロックを作成し、
操作の競合を防ぎます。`plan` は読み取り専用のため**ロックを取得しません**
（ロックの有無にかかわらず実行でき、他の操作をブロックすることもありません）。
...
- ロックは `apply` 終了時に自動解放されます。プロセスがクラッシュした等でロックが
  残った場合は `cmt force-unlock <host> <project>` で解除します。
- `apply` はロック取得時に project ディレクトリを作成します。
...
`--all --project grafana` のように絞り込んだ場合、その project を持たないホストは
エラーにせずスキップします。また、ロックファイルが壊れて読めない場合は単体指定と
同様に `--force` を付けたときのみ削除し、`--force` なしではエラーとして報告します。
