...
- そこで ansible の `falco_driver` ロール (`roles/falco_driver/`、docker ロール
  の後に実行) で Pi に
...
- `ansible/roles/falco_driver/defaults/main.yml` の `falco_driver_kmod_image` /
  `falco_driver_kmod_version`

## セットアップ / デプロイ

```sh
# 1. ホスト側モジュール (ansible)。falco_driver ロールが行う。
#    dkms 導入・ソース取得・ビルド/インストール・modules-load を行う。
make ansible-run            # site.yml (home_servers 経由で falco_driver ロール)
