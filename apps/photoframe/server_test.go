...
func testServer(t *testing.T, davURL string) *Server {
	...
	return NewServer(cfg, dav, slog.New(slog.NewTextHandler(io.Discard, nil)))
}
...
func TestHandleImages(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/images", nil))
...
func TestHandleImageProxies(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/img/"+photoID, nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200", rec.Code)
	}
	...
	rec = httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/img/deadbeef", nil))
	if rec.Code != http.StatusNotFound {
	...
	rec = httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/img/"+goneID, nil))
