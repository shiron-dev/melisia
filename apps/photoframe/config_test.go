...
func TestLoadConfigValidation(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			mutate:  func(t *testing.T) { t.Setenv("WEBDAV_BASE_URL", "") },
			wantErr: true,
		},
		{
			name:    "missing username",
			mutate:  func(t *testing.T) { t.Setenv("WEBDAV_USERNAME", "") },
			wantErr: true,
		},
		{
			name:    "missing password",
			mutate:  func(t *testing.T) { t.Setenv("WEBDAV_PASSWORD", "") },
			wantErr: true,
		},
		{
			name:    "cf access id without secret",
			mutate:  func(t *testing.T) { t.Setenv("CF_ACCESS_CLIENT_ID", "id-only") },
			wantErr: true,
		},
		{
			name:    "valid",
